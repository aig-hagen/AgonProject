...
This is an overview of how the repository is laid out. For setup and contributor workflows, see [Development setup](../contributing/development.md), [Extending AgonProject](../contributing/extending.md), and [Coding conventions](../contributing/conventions.md).

## Top level

- [`src/`](/src/) — the Vue frontend (see below)
- [`servers/`](/servers/) — auxiliary backend services (graph generation, sharing, and MCP)
...
- [`Dockerfile`](/Dockerfile) — builds the combined production image (frontend + TweetyProject + graph-gen + share and MCP servers, fronted by Caddy)
- [`CHANGELOG.md`](/CHANGELOG.md) — durable release history and the queue of unreleased changes
- [`docs/`](/docs/) — user, architecture, contributor, reference, and operations documentation,
  including the [`formats/`](../formats/README.md) reference documentation
...
- [`privacy/`](/src/app/privacy/) — the user-facing analytics and privacy notice
- [`usage/`](/src/app/usage/) — anonymous usage-event reporting
...
- `moduleConfig.ts` — the `ModuleConfig` registering the module (display name, initial content, save/load, examples) — see [Extending AgonProject](../contributing/extending.md#add-an-argumentation-module)
...
New argumentation types are added by following this same shape; see [Extending AgonProject](../contributing/extending.md) for the extension points in detail.
...
- [`window/`](/src/modules/common/window/), [`main-menu/`](/src/modules/common/main-menu/), [`settings/`](/src/modules/common/settings/), [`share/`](/src/modules/common/share/), [`notifications/`](/src/modules/common/notifications/), [`theme/`](/src/modules/common/theme/) — other shared editor chrome and app-wide concerns

## `servers/` — auxiliary backend services

The frontend talks to three backends in development (see [Development setup](../contributing/development.md) for how to run them locally), proxied via `vite.config.ts`:
...
[`argumentation-mcp/`](/servers/argumentation-mcp/) is an independent fourth service. It is not used by the browser or started by `scripts/dev.sh`; in production it exposes abstract argumentation reasoning and generation through MCP on internal port 8083.

In production all four services are bundled into one container ([`Dockerfile`](/Dockerfile)) and multiplexed behind Caddy ([`deployment/Caddyfile`](/deployment/Caddyfile)). See the [architecture overview](./overview.md) for their relationships and data flows.
