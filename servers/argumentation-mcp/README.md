...
| `ARGUMENTATION_MCP_ALLOWED_HOSTS`       | _(SDK localhost)_            | Comma list for Host checks; `*` disables (trust proxy)  |
| `ARGUMENTATION_MCP_ALLOWED_ORIGINS`     | _(SDK localhost)_            | Comma list for Origin checks                            |
| `ARGUMENTATION_MCP_RESOURCE_SERVER_URL` | _(unset)_                    | Public MCP URL; **required to enable auth**             |
| `ARGUMENTATION_MCP_STATIC_TOKEN`        | _(unset)_                    | Shared bearer token (Tier 1 auth)                       |
| `ARGUMENTATION_MCP_OAUTH_ISSUER`        | _(unset)_                    | OIDC issuer URL (Tier 2 auth)                           |
| `ARGUMENTATION_MCP_OAUTH_AUDIENCE`      | _(= resource URL)_           | Expected token audience (Tier 2)                        |
| `ARGUMENTATION_MCP_OAUTH_JWKS_URL`      | _(discovered)_               | JWKS endpoint; else found via issuer discovery (Tier 2) |
| `ARGUMENTATION_MCP_OAUTH_ALGORITHMS`    | `RS256,ES256`                | Accepted JWT signing algorithms (Tier 2)                |
| `ARGUMENTATION_MCP_REQUIRED_SCOPES`     | _(none)_                     | Comma list of scopes a token must carry                 |
...
only when `ARGUMENTATION_MCP_RESOURCE_SERVER_URL` is set together with at least one credential
...
**Tier 1 — shared static token (default choice).** Set
`ARGUMENTATION_MCP_STATIC_TOKEN` to a secret
...
**Tier 2 — OAuth 2.1 / OIDC.** Set `ARGUMENTATION_MCP_OAUTH_ISSUER`
(provider-agnostic). Bearer
tokens are validated as OIDC JWTs: signature via the issuer's JWKS (discovered
from the issuer, or set `ARGUMENTATION_MCP_OAUTH_JWKS_URL`), plus issuer,
audience, and expiry.
...
at once (a valid static token _or_ a valid JWT is accepted);
`ARGUMENTATION_MCP_REQUIRED_SCOPES` tightens Tier 2.
