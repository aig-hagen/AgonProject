...
const activeExtensionId = computed<string | undefined>({
  get: () => (activeWindow.value?.source === 'extension' ? activeWindow.value.id : undefined),
  set: (id) => {
    activeWindow.value = id === undefined ? undefined : { source: 'extension', id }
  },
})
